...
sync_pull() {
  if [[ -d .git ]]; then
    ...
    git pull --rebase || { 
    echo "⚠️ Conflicto al sincronizar, revisar manualmente"
...
sync_push() {
  if [[ -d git ]]; then
    git add datos_tanda/*.csv
    git commint -m "Actualización tanda $(date '+%F %H:%M')" || true
    git push || echo "⚠️ No se pudo subir, revisa conexión"
  fi
}
# ==============================
...
menu_principal() {
    while true; do
        ...
        case "$op" in
            ...
            0) echo "Guardando y sincronizando.."
              sync_push
              exit 0 ;;
